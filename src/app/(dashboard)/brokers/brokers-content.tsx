...
import { brokerNameToBrokerType } from '@/lib/broker-mapping';
...
export function BrokersContent({ initialConnections, initialPagination, accounts }: BrokersContentProps) {
  ...
  // Available brokers from API (Story 3.9: Dynamic Broker Dropdown)
  const [availableBrokers, setAvailableBrokers] = useState<Array<{ name: string; displayName: string | null; brokerType: BrokerType }>>([]);
  const [loadingBrokers, setLoadingBrokers] = useState(true);
  
  // Load available brokers from API
  useEffect(() => {
    const fetchAvailableBrokers = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          // Map brokers to BrokerType and filter out unmappable ones
          const mappedBrokers = data.data
            .map((broker: { name: string; displayName: string | null }) => {
              const brokerType = brokerNameToBrokerType(broker.name);
        ...
        setAvailableBrokers([
          { name: 'Interactive Brokers', displayName: 'Interactive Brokers', brokerType: 'IBKR' },
        ]);
      } finally {
  ...
  const handleLinkAccount = async (connectionId: string, accountId: string | null) => {
    setUpdatingSettings(prev => new Set(prev).add(connectionId));
    try {
  ...
  const handleToggleSyncEnabled = async (connectionId: string, enabled: boolean) => {
    ...
    try {
      ...
      toast({
        ...
        description: enabled ? t('autoSyncEnabled') : t('autoSyncDisabled'),
      });
      router.refresh();
    } catch (error) {
      toast({
  ...
  const handleUpdateSyncInterval = async (connectionId: string, intervalMin: number) => {
    ...
    try {
      ...
      toast({
        title: t('settingsUpdated'),
        description: t('syncIntervalUpdated'),
      });
      router.refresh();
    } catch (error) {
      toast({
      ...
      {connections.length === 0 ? (
        ...
        <div className="grid gap-4">
          {connections.map((connection) => {
            ...
            const isUpdating = updatingSettings.has(connection.id);
              ...
              <Card key={connection.id}>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {connection.syncEnabled ? t('autoSyncEnabledDesc') : t('autoSyncDisabledDesc')}
                        </p>
                      </div>
                    </div>
                    <Switch
                  ...
                  <div className="border-t pt-4">
                    <button
                      onClick={() => toggleConnectionDetails(connection.id)}
                ...
                <SelectContent>
                  {loadingBrokers ? (
                    ...
                    availableBrokers.map((broker) => {
                      const brokerInfo = BROKER_INFO[broker.brokerType];
                      return (
                        <SelectItem key={broker.brokerType} value={broker.brokerType}>
                          <div className="flex items-center gap-2">
                            <span>{brokerInfo?.logo || '📊'}</span>
                            <span>{broker.displayName || broker.name}</span>
                          </div>
                        </SelectItem>
                      );
                    })
